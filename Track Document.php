...
if ($pdo instanceof PDO) {
    try {
        ...
        if (!empty($check)) {
            $stmt = $pdo->query("SELECT tracking_id, fullname AS name, document_type AS type, status, COALESCE(progress,0) AS progress, DATE_FORMAT(created_at, '%Y-%m-%d') AS date, COALESCE(remarks,'') AS remarks FROM requests ORDER BY created_at DESC");
    ...
    if ($res = $mysqli->query("SHOW TABLES LIKE 'requests'")) {
        if ($res->num_rows > 0) {
            $res->free();
            $q = "SELECT tracking_id, fullname AS name, document_type AS type, status, COALESCE(progress,0) AS progress, DATE_FORMAT(created_at, '%Y-%m-%d') AS date, COALESCE(remarks,'') AS remarks FROM requests ORDER BY created_at DESC";
            if ($r = $mysqli->query($q)) {
...
// Fallback demo data when DB not available or table empty
if (empty($rows)) {
    $rows = [
      ['tracking_id'=>'TRK-001','name'=>'John Doe','type'=>'Birth Certificate','status'=>'Processing','progress'=>60,'date'=>'2026-05-01'],
      ['tracking_id'=>'TRK-002','name'=>'Jane Smith','type'=>'Marriage License','status'=>'Completed','progress'=>100,'date'=>'2026-05-02'],
      ['tracking_id'=>'TRK-003','name'=>'Bob Johnson','type'=>'Business Permit','status'=>'Pending','progress'=>20,'date'=>'2026-05-03'],
      ['tracking_id'=>'TRK-004','name'=>'Alice Brown','type'=>'Tax Clearance','status'=>'Processing','progress'=>75,'date'=>'2026-05-04'],
      ['tracking_id'=>'TRK-005','name'=>'Charlie Davis','type'=>'ID Card','status'=>'Pending','progress'=>10,'date'=>'2026-05-05'],
      ['tracking_id'=>'TRK-006','name'=>'Sofia Reyes','type'=>'Land Title','status'=>'Completed','progress'=>100,'date'=>'2026-05-06'],
      ['tracking_id'=>'TRK-007','name'=>'Mark Torres','type'=>"Driver's License",'status'=>'Processing','progress'=>45,'date'=>'2026-05-07'],
    ];
}
?><!DOCTYPE html>
<html lang="en">
...
<header class="topbar">
  ...
  <div class="topbar-right">
    <button class="bell-btn" title="Notifications">🔔<span class="bell-dot"></span></button>
    <div class="user-chip">
...
<script>
  ...
  function applyStatusUpdate() {
    ...
    const progress = parseInt(document.getElementById('modalProgress').value) || null;

    const doc = documents.find(d => d.id === id);
    if (doc) {
      doc.status = newStatus;
      if (progress !== null && !isNaN(progress)) doc.progress = Math.min(100, Math.max(0, progress));
    }
